...
- 撰写文章，上传到github会导致图片丢失，不可见

  解决： 进行文件资源引用，官方文档https://hexo.io/zh-cn/docs/asset-folders

- 如何新建文件夹，不止是在`source/_post/`下

  解决：`hexo new page test` 会生成`source/test/`文件夹，但这个无法再`test`下加其他md文件，无法识别
